#!/bin/bash
# LLMunix Agent Setup Script for Qwen Code
# This script copies all agent markdown files to the .qwen/agents directory to make them discoverable by Qwen Code

# Create the .qwen/agents directory if it doesn't exist
mkdir -p .qwen/agents
...
if [ -d "system/agents" ]; then
    for agent in system/agents/*.md; do
        if [ -f "$agent" ]; then
            dest_path=".qwen/agents/$(basename "$agent")"
...
echo "Agent setup complete. Agents are now discoverable by Qwen Code."
echo "Available agents:"
for agent in .qwen/agents/*.md; do
    if [ -f "$agent" ]; then
